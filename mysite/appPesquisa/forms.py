...
class PesquisadorForm(forms.ModelForm):
    ...
    tipo = forms.ChoiceField(label='Tipo de pesquisador', widget=forms.Select(
        attrs={'class': 'border border-gray-300 rounded px-4 py-2'}))
    area_atuacao = forms.CharField(label='Área de atuação', max_length=100, widget=forms.TextInput(
        attrs={'class': 'border border-gray-300 rounded px-4 py-2'}))
    class Meta:
        model = Pesquisador
